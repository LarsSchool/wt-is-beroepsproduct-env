...
$data .= '</thead>';
    ...
    if($affected_rows == 0){
      echo 'Dit vluchtnummer staat helaas niet in onze database, excuses hiervoor.';
    }
    else {
    foreach ($stmt as $rij)
    {
...
{ 
 ...
 $bool_sort_by = true;
  ...
  if($bool_show_all_passagiers){
    ...
    if($affected_rows == 0){
      echo 'Dit passagiernummer staat helaas niet in onze database, probeer een andere.';
    }
    else {
    foreach ($stmt as $rij)
    {
